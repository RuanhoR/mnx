...
const language: {
	...
	zh: {
		...
		HeaderToDocs: '文档',
		AboutContent:
			'pmnx 是一个mcbe模组管理平台（由Github @RuanhoR 编写，MIT开源），由同时是mcx dsl的cli mbler作为快速使用的cli，可以以一个cli命令，将模组下载到游戏文件夹，快速地启动和运行在mcbe的游戏里面。想使用？可见导航栏文档和导航栏下载游戏链接。最后，祝你能帮助本社区壮大和找到自己想要的模组，开发顺利',
		AboutTitle: '关于 PMNX',
		Copied: '复制成功',
		HeaderToAccount: '账号',
		Search: '搜索',
		NotFound: '未找到资源... (6^=^6) 可能是你访问了错误的链接或者资源已经被删除了。',
		Searching: '搜索中...',
		Found: '找到',
		results: '个结果',
		NoResultsFound: '未找到结果',
		SearchWelcome: '输入关键字搜索包',
		Version: '版本',
		Downloads: '下载',
		Package: '包',
		AllVersion: '所有版本',
	...
	en: {
		...
		SearchWelcome: 'Enter keywords to search for packages',
		Version: 'Version',
		Downloads: 'Downloads',
		Package: 'Package',
		Readme: 'README',
		...
		AboutContent:
			'pmnx is an mcbe mod management platform (written by Github @RuanhoR, MIT open source), and also has a CLI called mbler, which is the CLI for mcx dsl, for quick use. With a single CLI command, you can download mods to the game folder and quickly launch and run them in the mcbe game. Want to use it? See the documentation in the navigation bar and the game download link in the navigation bar.Finally, I wish you can help this community grow, find the mods you want, and have smooth development.',
		...
		NotFound: 'Not Found Asset... (6^=^6) Maybe you are visiting a wrong link or the asset has been deleted.',
		// token management
		...
		TokenScope: 'Wildcard Package (* represents all)',
