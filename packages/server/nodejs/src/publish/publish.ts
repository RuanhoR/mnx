...
import { KV, DB, Storage } from '../framework';
...
export class PublishManager extends PublishToken {
	...
	private static async findPackage(scope: string, name: string): Promise<MNXPackageData | null> {
		...
		const exact = await DB.packages.select('*').eq('scope', normalizedScope).eq('name', normalizedName).limit(1);
		...
		const fuzzy = await DB.packages.select('*').ilike('name', name.trim()).limit(50);
		if (fuzzy.error) {
	...
	static async packageInfo(scope: string, name: string): Promise<MNXPackageInfoResult> {
		try {
			...
			const kvCacheData = await KV.get(`mnx-packages:cache:${normalizedScope}/${normalizedName}`);
			if (kvCacheData) {
			...
			for (const version of packageData.versions) {
				const readmeResult = await DB.readmes.select('content').eq('id', version.readme).limit(1);
			...
			const versions = await Promise.all(
				packageData.versions.map(async (version) => {
					const userResult = await DB.users.select('uid, name, mail').eq('uid', version.create_user).limit(1);
			...
			await KV.put(`mnx-packages:cache:${normalizedScope}/${normalizedName}`, JSON.stringify(returnData));
			return returnData;
		} catch (error) {
	...
	static async publishPackage(
		...
		try {
			...
			const scopeResult = await DB.scopes.select('*').eq('name', scope).limit(1);
			...
			if (await KV.get(`mnx-packages:cache:${scope}/${name}`)) await KV.delete(`mnx-packages:cache:${scope}/${name}`);
	...
	static async unpublishPackage(
		...
		try {
			// Find package
			const packageResult = await DB.packages.select('*').eq('scope', scope).eq('name', name).limit(1);
	...
	static async publish(user: User, metadata: PublishMetadata, file: File): Promise<BaseResult> {
		...
		return await KVLockManager.executeWithLock(metadata.scope, metadata.name, async () => {
			try {
				...
				const scopeResult = await DB.scopes.select('*').eq('name', metadata.scope).limit(1);
				if (!scopeResult.data || scopeResult.data.length === 0) {
					return { code: -1, message: 'Scope does not exist', success: false };
				}
	...
	static async listPackages(user: User): Promise<ListPackageResult> {
		try {
			...
			const scopeResult = await DB.scopes.select('name').eq('user', user.uid);
			...
			const packagesResult = await DB.packages.select('id, scope, name, download').in('scope', userScopes);
			if (!packagesResult.data || packagesResult.data.length === 0) {
				return { code: 200, message: 'No packages found', success: true, data: [] };
			}
	...
	private static async handleReadmePublish(readmeContent: string): Promise<number> {
		...
		const existingReadme = await DB.readmes.select('id, cout').eq('content', readmeContent).limit(1);
		...
		if (existingReadme.data && existingReadme.data.length > 0) {
			...
			await DB.readmes.update({ cout: readme.cout + 1 }).eq('id', readme.id);
			return readme.id;
		} else {
			// Create new README record
			const newReadme = await DB.readmes.insert({ content: readmeContent, cout: 1 }).select('id');
	...
	private static async handleReadmeUnpublish(readmeId: number): Promise<void> {
		const readmeResult = await DB.readmes.select('cout').eq('id', readmeId).limit(1);

		if (readmeResult.data && readmeResult.data.length > 0) {
			const currentCount = readmeResult.data[0].cout;
			if (currentCount <= 1) {
				// Reference count is 1 or less, delete record
				await DB.readmes.delete().eq('id', readmeId);
			} else {
				// Decrease reference count
				await DB.readmes.update({ cout: currentCount - 1 }).eq('id', readmeId);
			}
		}
	}
	...
	static async downloadVersion(scope: string, name: string, version: string): Promise<{ success: boolean; data?: ReadableStream; error?: string; size?: number }> {
		try {
			...
			const result = await Storage.createReadableStream(filePath);
			if ('error' in result) {
			...
			const existingCache = await KV.get(cacheKey);
			if (existingCache) {
				const parsed = JSON.parse(existingCache);
				parsed.download = packageData.download + 1;
				await KV.put(cacheKey, JSON.stringify(parsed));
			}
	...
	static async getDownloadStats(scope: string, name: string): Promise<{ success: boolean; downloads?: number; error?: string }> {
		try {
			const packageResult = await DB.packages.select('download').eq('scope', scope).eq('name', name).limit(1);
