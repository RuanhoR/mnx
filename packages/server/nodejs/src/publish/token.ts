...
export class PublishToken {
	static async verifyToken(token: string, mode: TokenPermission, scope: string): Promise<User | null> {
		const result = await DB.publishTokens.select('token,permission,scope,user,id,time,created_at').eq('token', token).limit(1);
		...
		if (tokenData.time && new Date(tokenData.created_at).getTime() + tokenData.time < Date.now()) {
			...
			await DB.publishTokens.delete().eq('id', tokenData.id);
			return null;
		}
		...
		const userResult = await DB.users.select('*').eq('uid', tokenData.user).limit(1);
	...
	static async deleteToken(params: FindTokenParam): Promise<boolean> {
		let query = DB.publishTokens.delete();

		if (params.id) {
			query = query.eq('name', params.id);
		}
		if (params.content) {
	...
	static async listToken(user: User): Promise<MNXPublishToken[]> {
		const result = await DB.publishTokens.select('*').eq('user', user.uid);
	...
	static async setToken(params: FindTokenParam, updateData?: UpdateTokenParams): Promise<MNXPublishToken | null> {
		let query = DB.publishTokens.update(updateData || {});
