import ResponseFrame, { DB } from "../framework";
...
async function isScopeNameTaken(name: string): Promise<boolean> {
  const result = await DB.scopes.select("id").eq("name", name).limit(1);
...
async function getMyScope(data: Map<string, string>, request: Request, url: URL): Promise<Response> {
  try {
    ...
    const scopeResult = await DB.scopes.select("*").eq("user", user.uid).limit(1);
...
async function setScope(data: Map<string, string>, request: Request, url: URL): Promise<Response> {
  try {
    ...
    const existingScope = await DB.scopes.select("id, name").eq("user", user.uid).limit(1);
    ...
    const nameExistsResult = await DB.scopes.select("id").eq("name", scopeName).limit(1);
    ...
    let result;
    if (isNewScope) {
      result = await DB.scopes.insert({
        user: user.uid,
        name: scopeName,
        created_at: new Date()
      }).select();
    } else {
      result = await DB.scopes.update({
        name: scopeName
