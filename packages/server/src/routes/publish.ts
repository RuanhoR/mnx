...
const uploadMetadata: HandlerFn = async (data, request, _) => {
	...
	try {
		...
		if (metadata.name.length < 1 || !/[a-zA-Z0-9_]/.test(metadata.name))
			return json(
				{
					code: -1,
					data: 'Bad name',
...
const ListPackage: HandlerFn = async (data, request, _) => {
	...
	if (result.success && result.data) {
		await KV.put(cacheKey, JSON.stringify(result.data), { expirationTtl: 60 * 5 });
	}
...
const downloadPackage: HandlerFn = async (data, request, _) => {
	...
	try {
		...
		if (downloadResult.size) {
			headers['Content-Length'] = downloadResult.size.toString();
		}
