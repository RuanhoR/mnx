...
async function genToken(data: Map<string, string>, request: Request, url: URL): Promise<Response> {
	try {
		...
		// Create the token
		const createdToken = await PublishToken.createToken(name, user.uid, permissionEnums, scopes, expirationTime);
...
async function deleteToken(data: Map<string, string>, request: Request, url: URL): Promise<Response> {
	try {
		...
		// Delete the token
		const result = await PublishToken.deleteToken({ id: tokenName });
